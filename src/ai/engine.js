...
async function init(callbackFn_, loadFullEngine) {
  ...
  const effectiveLoadFullEngine = isCapacitor ? true : loadFullEngine

  const engineFlags =
    (supportThreads ? '-multi' : '-single') +
    (supportSIMD && effectiveLoadFullEngine && !isCapacitor ? '-simd128' : '') +
    (supportRelaxedSIMD && effectiveLoadFullEngine && !isCapacitor ? '-relaxed' : '')
  const engineDir = effectiveLoadFullEngine ? '/' : '/fallback/'
